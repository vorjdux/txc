...
mod tests {
    ...
    ///
    /// The column is counted in characters, not bytes: the borders are multi
    /// byte, so a byte offset would not be a screen position.
    ...
    #[test]
    fn the_input_cursor_follows_the_text() {
        let mut app = App::new();
        app.search = "upper".to_string();
        app.refresh_operations();
        app.load_operation();
    ...
    #[test]
    fn the_search_cursor_follows_the_text() {
        let mut app = App::new();
